...
TWO QUESTIONS PER CELL, and they are not the same question:
  READ   — can the model get the words off the page?          (perception; we have data for some cells)
  GRADE  — does it mark the page like a teacher would?        (agreement; needs teacher-marked pages)
A perfect READ score with no GRADE score is a research result, not a product.
...
# Teacher-marked pages: the only thing that can answer the GRADE question. Declared here so that the day
# they exist, the scoreboard picks them up without an edit.
TEACHER_MARKS = {
...
for c in CELLS:
    ...
    tm = TEACHER_MARKS.get((c["type"], c["lang"]))
    rows.append({
        ...
        "grade_gt": bool(tm) and exists(tm),
        "grade_score": None,                        # nothing can produce this until teacher marks exist
        "note": c["note"],
    })
...
summary = {
    ...
    "with_grade_test": sum(r["grade_score"] is not None for r in rows),
}
out = {"summary": summary, "rows": rows,
       "headline": "READ says the words can be lifted off the page. GRADE says it marks like a teacher. "
                   "No cell has a GRADE number, so no cell is production grade yet."}
...
W = (19, 6, 8, 10, 9, 41, 9)
hdr = ("type", "lang", "engine", "contract", "read GT", "READ score (measured)", "GRADE")
...
for r in rows:
    print("".join(x.ljust(w) for x, w in zip((
        ...
        "no test" if r["grade_score"] is None else r["grade_score"],
    ), W)))
print("-" * sum(W))
print(f"\n  engines built      : {summary['with_engine']} of {summary['cells']}")
print(f"  cells with a READ  : {summary['with_read_test']} of {summary['cells']}")
print(f"  cells with a GRADE : {summary['with_grade_test']} of {summary['cells']}   <- what 'production grade' means")
print("\nnotes:")
for r in rows:
    if r["note"]:
